...
vogais = 'aáàãäâeéèẽëêiíìĩïîoóòõöôuúùũüû'
consoantes = 'abcçdefghijklmnopqrstuwvxyz'

letra = letra.lower()

totalVogal = 0
totalConsoantes = 0
totalJoao = letra.count('joão')
totalJoao = totalJoao + letra.count('joao')
totalCaracteres = len(letra)


for caracter in letra:
    if caracter in vogais: # conta veriavel
        totalVogal = totalVogal + 1
    elif caracter in consoantes: # conta consoantes
        totalConsoantes = totalConsoantes + 1

print(f"Total de Vogais: {totalVogal}")
print(f"Total de Consoantes: {totalConsoantes}")
print(f"Total de Caracteres: {totalCaracteres}")
print(f"Total que aparece a palavra João: {totalJoao}")
